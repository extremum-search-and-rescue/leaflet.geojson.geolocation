namespace L {
    export interface GeolocationMarkerOptions extends L.CircleMarkerOptions {
        iconClassName?: string
    }

    export class GeolocationMarker extends L.CircleMarker {
        override options: L.GeolocationMarkerOptions;
    ...
    export function geolocationMarker(latlng?: L.LatLngExpression, options?: GeolocationMarkerOptions) {
        return new L.GeolocationMarker(latlng, options);
    }

    export class GeolocationLayer extends L.GeoJsonLayer {
        ...
        pointToLayer (feature: GeoJSON.Feature<GeoJSON.Point,any>, latlng: L.LatLng) {
            ...
            const utcDate = new Date(0); utcDate.setUTCMilliseconds(feature.properties.time);
